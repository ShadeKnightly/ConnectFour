...
public abstract class Player : IPlayer
{
    ...
    protected Player(string name, Color col) // no need for score, it will be set to 0 by default
    {
        Name = name;
        TokenColor = col;
        Score = 0;
...
public class SmartAIPlayer : AIPlayer
{
    ...
    private bool SimulateMove(Button[,] board, int column, Color color)
    {
        for (int row = board.GetLength(0) - 1; row >= 0; row--)
        {
            if (board[row, column].BackColor == Color.White)
            {
                ...
                board[row, column].BackColor = Color.White; // Undo move
    ...
    {
        return board[0, column].BackColor == Color.White; // Ensure column isn’t full
    }
