...
namespace connect4UI
{
    ...
    {
        private Button[,] board = new Button[6, 7];
        private bool IsGameOver = false;
        private Player currentPlayer, player1, player2;
        private int gameMode = 0;
        private Form titleForm;

        public Connect4PvP(Form titleForm, int gameMode = 0)
        {
            InitializeComponent();
        ...
        {
            // Safely show the title form if it was passed in
            if (titleForm != null)
            {
                titleForm.Show();
            }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            foreach (var button in new[] { Col0Btn, Col1Btn, Col2Btn, Col3Btn, Col4Btn, Col5Btn, Col6Btn })
            {
                ...
                button.MouseHover -= MouseColHover;
                button.MouseHover += MouseColHover;
        ...
        {
            var randomPlayer = RandomNumberGenerator();
            string strGameMode = "";
            ...
            else
            {
                ...
                {
                    case 0:
                        player1 = new HumanPlayer("Yellow Player", Color.Yellow);
                        strGameMode = "Human v Human";
                        break;
                    case 1:
        ...
        private bool MakeMove(int column)
        {
            for (int row = 5; row >= 0; row--) // start from bottom row
            {
                ...
                if (board[row, column].BackColor == Color.Black)
                {
                    ...
                    board[row, column].BackColor = currentPlayer.TokenColor;  

                    // Set the background image to show the player's token
                    if (currentPlayer.TokenColor == Color.Red)
                    {
                        board[row, column].BackgroundImage = Resources.RedToken;
                    }
                    else
                    {
                        board[row, column].BackgroundImage = Resources.YellowToken;
                    }
        ...
        private new void BackgroundImageChanged(object sender, EventArgs e)
        {

        }

        private void MouseColHover(object sender, EventArgs e)
        {
            Button hoveredButton = (Button)sender;

            int column = Convert.ToInt32(hoveredButton.Tag);

            // Change to Red or Yellow token depending on current player
            hoveredButton.BackgroundImage = currentPlayer.TokenColor == Color.Red
                ? Resources.RedToken
                : Resources.YellowToken;

            hoveredButton.BackgroundImageLayout = ImageLayout.Stretch;
        }

        private void MouseColLeave(object sender, EventArgs e)
        {
            Button hoveredButton = (Button)sender;
        ...
        private void Connect4PvP_Leave(object sender, EventArgs e)
        {
            titleForm.Show();
        }
